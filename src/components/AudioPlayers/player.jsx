    ...
    <Box sx={{ width: '100%', overflow: 'hidden', position: 'relative', p: 3 }}>
      <Widget>
        <Box sx={{ display: 'flex', alignItems: 'center' }}>
          <CoverImage>
            <img
              ...
              src={currentSong? currentSong.thumbnail:spotify_logo}
            />
          </CoverImage>
          <Box sx={{ ml: 1.5, minWidth: 0 }}>
            <Typography
              variant="caption"
              sx={{ color: '#F1FAEE', fontWeight: 500 }}
            >
              Coolie
            </Typography>
            <Typography noWrap sx={{color:'#000'}}>
              <b> {currentSong? currentSong.songName : "Listen to Spotify.."}</b>
            </Typography>
            <Typography noWrap sx={{ letterSpacing: -0.25 }}>
              {currentSong?currentSong.songName:"vibe until you get vibed from spotify"}
            </Typography>
          </Box>
        </Box>
        <Slider
        ...
        >
          ...
          >
            {paused ? (
              <PlayArrowRounded  onClick={()=>playSong(currentSong.id)} sx={{ fontSize: '3rem' }} />
            ) : (
